...
def render_scrape_time_module(scraped_at: str) -> str:
    """Render web/src/lib/generated/scrape-time.ts. ISO value needs no escaping."""
    lines = [
        "/**",
        " * Oldest scrape time across published subjects, so it holds for all of them.",
        " * Auto-generated by scripts/publish_course_data.py.",
        " */",
        f"export const SCRAPED_AT: string = '{scraped_at}'",
    ]
    return "\n".join(lines) + "\n"
