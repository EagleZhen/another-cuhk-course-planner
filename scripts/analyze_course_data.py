                        ...
                        if time_str and time_str.upper() != "TBA":
                            ...
                            if start_h != -1:  # Valid time
                                ...
                                if start_h == end_h and start_m == end_m:
                                    ...
                                    if (
                                        current_hour not in hourly_examples
                                        or hourly_counts[current_hour]
                                        > hourly_examples[current_hour]["count"]
                                    ):
...
def analyze_weekend_courses(subjects_data: dict[str, any]) -> dict[str, any]:
    ...
    weekend_patterns = [
        r"\bSa\b",
        r"\bSat\b",
        r"\bSaturday\b",  # Saturday patterns
        r"\bSu\b",
        r"\bSun\b",
        r"\bSunday\b",  # Sunday patterns
    ]
...
def analyze_class_vs_course_attributes(subjects_data: dict[str, any]) -> dict[str, any]:
            ...
            for term in course.get("terms", []):
                for section in term.get("schedule", []):
                    ...
                    if course_attrs and class_attrs:
                        ...
                        class_lines = [
                            line.strip() for line in class_attrs.split("\n") if line.strip()
                        ]
                        course_lines = [
                            line.strip() for line in course_attrs.split("\n") if line.strip()
                        ]

                        # Find lines in class_attrs that are NOT in course_attrs
                        class_specific_lines = [
                            line for line in class_lines if line not in course_lines
                        ]
                                ...
                                cleaning_examples.append(
                                    {
                                        ...
                                        "cleaned": cleaned,
                                        "lines_removed": len(class_lines)
                                        - len(class_specific_lines),
    ...
    if sections_with_both_attrs > 0:
        ...
        print(
            f"  ✅ Successfully cleaned: {rule_followers:,} ({rule_followers / sections_with_both_attrs * 100:.1f}%)"
        )
        ...
        print(
            f"  ❌ No overlap found: {rule_violations:,} ({rule_violations / sections_with_both_attrs * 100:.1f}%)"
        )
        print()
        ...
        for ex in complex_examples:  # Show ALL complex cases
            ...
            if ex["examples"]:
                print(
                    f"     📚 ALL Examples ({len(ex['examples'])} courses): {', '.join(sorted(set(ex['examples'])))}"
                )
            print()
        ...
        if complex_examples:
            ...
            for level, count in level_distribution.items():
                if count > 0:
                    percentage = (
                        count / total_complex_courses * 100 if total_complex_courses > 0 else 0
                    )
                    level_type = (
                        "Undergraduate"
                        if level in ["1000s", "2000s", "3000s", "4000s"]
                        else "Postgraduate"
                        if level == "5000s+"
                        else "Unknown"
                    )
            ...
            undergrad_count = sum(
                [level_distribution[k] for k in ["1000s", "2000s", "3000s", "4000s"]]
            )

            if total_complex_courses > 0:
                print()
                print(
                    f"   🎓 Summary: {undergrad_count}/{total_complex_courses} ({undergrad_count / total_complex_courses * 100:.1f}%) Undergraduate"
                )
                print(
                    f"   🎓 Summary: {postgrad_count}/{total_complex_courses} ({postgrad_count / total_complex_courses * 100:.1f}%) Postgraduate"
                )
            print()
    ...
    try:
        ...
        for stype in sorted(section_types.keys()):
            ...
            if stype in section_examples:
                ...
                print(
                    f"  📚 Example: {example['subject']}{example['course_code']} - {example['course_title']}"
                )
        ...
        if earliest_example:
            print(
                f"   📚 Example: {earliest_example['course_code']} - {earliest_example['course_title']}"
            )
        ...
        if latest_example:
            print(
                f"   📚 Example: {latest_example['course_code']} - {latest_example['course_title']}"
            )
        ...
        print(
            f"🔥 Peak hours: {[f'{h:02d}:00' for h in peak_hours]} ({hourly_analysis['peak_count']} sections)"
        )
        print(
            f"😴 Quietest hours: {[f'{h:02d}:00' for h in quiet_hours]} ({hourly_analysis['quiet_count']} sections)"
        )
        print()
        ...
        print(
            f"   Coverage: {req_analysis['courses_with_requirements'] / req_analysis['total_courses'] * 100:.1f}%"
        )
        print()
        ...
        print("📊 Weekend Course Statistics:")
        ...
        print(
            f"   Percentage: {(weekend_count / total_analyzed * 100):.2f}% of courses have weekend classes"
        )
        print()
        ...
        sorted_subjects = sorted(
            subject_stats.items(), key=lambda x: x[1]["total_courses"], reverse=True
        )
        ...
        for subject, stats in sorted_subjects[:15]:
            ...
            print(
                f"  {display_name:<35} : {stats['total_courses']:>3} courses, {stats['total_sections']:>4} sections"
            )
        ...
        print(
            f"   - Suggested start: {max(7, earliest[0] - 1):02d}:00 (earliest class at {earliest[0]:02d}:{earliest[1]:02d})"
        )
        print(
            f"   - Suggested end: {min(23, latest[0] + 1):02d}:00 (latest class ends at {latest[0]:02d}:{latest[1]:02d})"
        )
        print()
