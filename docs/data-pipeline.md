...
- section codes — each must read as `<label>-<COMP> (<classNbr>)`, and a label carrying digits must have room for both a cohort and its type letter
- course terms — one where no single cohort covers every type aborts, stricter than the app on purpose ([grammar](decisions.md#read-the-cohort-from-the-section-code-grammar))
