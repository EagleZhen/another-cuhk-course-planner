...
- **Keyword search** covers code, title, and current-term instructors via one shared `courseMatchesKeyword`. Instructors match on both the scraped name and the compact form shown on the card, so a query finds whichever the user saw. Course descriptions are display content, not a matching field; they can mention prerequisites or related courses that are not the course the user searched for.
