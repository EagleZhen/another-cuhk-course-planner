 ...
 * Matches course code (e.g. "csci1020" or just "1020"), title, and instructor
 * names in the given term.
 */
export function courseMatchesKeyword(course: InternalCourse, query: string, term: string): boolean {
  ...
  if (fullCode.includes(query) || course.title.toLowerCase().includes(query)) {
    return true
  }
