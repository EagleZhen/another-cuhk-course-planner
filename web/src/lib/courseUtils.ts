...
/**
 * Determine which badges to show based on section status and availability
 */
export function getAvailabilityBadges(availability: SectionAvailability) {
  ...
  // 1. Section Status Badge (leftmost, most important)
  badges.push({
    type: 'status' as const,
    text: status,
    style: getSectionStatusStyle(status),
  })
...
/**
 * Get section status badge styling based on status - intuitive warning system
 */
function getSectionStatusStyle(status: string) {
  switch (status) {
    case 'Open':
      return {
...
/**
 * Process ICS file content to add STATUS:CANCELLED to all events
 * Used for undoing calendar imports
 * @param content ICS file content
 * @returns Object with modified content or validation result
 */
export function processICSForUndo(content: string): {
  ...
  if (!content.includes('BEGIN:VCALENDAR')) {
    return { success: false, error: 'Invalid ICS file: missing VCALENDAR structure' }
  }

  // Check if file was generated by our app
